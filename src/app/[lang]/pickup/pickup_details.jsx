...
export default function PickupDetails({
  pickupDict, pickup, destination, pickupCoords, destinationCoords,
  phone, pickupDate, pickupTime, price, returnPrice, numAdultSeats,
  numChildSeats, customerNote, returnDate, returnTime,
  vehicleID, vehicleCategory, login, signup, lang
}) {
  ...
  const [pickupData, setPickupData] = useState({
    ...
    returnDate, returnTime, customerNote
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (response.status === 201) {
      ...
      if (!isGuest) {
        ...
        router.push(
          `/${lang}/pickup-details?pickup=${pickupData.pickup}&destination=${pickupData.destination}&phone=${pickupData.phone}&pickupDate=${pickupData.pickupDate}&pickupTime=${pickupData.pickupTime}&price=${totalPrice}&returnDate=${pickupData.returnDate}&returnTime=${pickupData.returnTime}&vehicle=${pickupData.vehicleCategory}`
        );
      }
          ...
          <div className="space-y-4">
                ...
                <div className="flex items-center space-x-3">
                  <div>
                    ...
                    <p className="text-gray-600">
                      {pickupData.pickup || "Not provided"}
                    </p>
                  </div>
                </div>
                {pickupData.destination && (
                  <div className="flex items-center space-x-3">
                    <div>
                      ...
                      <p className="text-gray-600">
                        {pickupData.destination || "Not provided"}
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className="w-full flex justify-between items-center pr-2 rounded-lg border-gray-200">
              ...
              <p className="text-orange-600 text-xl font-bold">
                €
                {Number(pickupData.price) +
                  Number(pickupData.returnPrice || 0)}
              </p>
            </div>
            ...
            <div>
              <h3 className="font-bold text-gray-900 text-lg">
                {pickupDict.paymentMethod}
              </h3>
              <RadioGroup
                defaultValue="cash"
                onValueChange={(value) => setPaymentCash(value)}
                className={"mt-3"}
              >
                <div
                  className={`flex items-center gap-3 p-3 rounded-lg border border-gray-200 ${
                    paymentCash === "cash" ? "border-2 border-gray-700" : ""
                  }`}
                >
                ...
                <div
                  className={`flex items-center gap-3 p-3 rounded-lg border border-gray-200 ${
                    paymentCash === "credit" ? "border-2 border-gray-700" : ""
                  }`}
                >
