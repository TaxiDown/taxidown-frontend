...
const GEOAPIFY_API_KEY = '8095bc6a07e74818bff959b1ebbce386';
    ...
    useEffect(() => {
      ...
      const fetchSuggestions = async () => {
        const url = `https://api.geoapify.com/v1/geocode/autocomplete?text=${encodeURIComponent(
          pickupQuery
        )}&filter=countrycode:es&lang=es&apiKey=${GEOAPIFY_API_KEY}&limit=5`;
                ...
                <div className="absolute bg-[#fcfcfa] border border-gray-300 rounded shadow w-full top-[39] z-10">
                    {destinationResults.map((place, idx) => (
                        <div
                            key={idx}
                            onClick={() => {
                                setdestinationQuery(place.properties.formatted);
                                setShowDestinationResults(false);
                            }}
                            className="p-2 hover:bg-gray-100 cursor-pointer"
                        >
