...
export default function BookingForm({pickup, destination, dict}) {
  ...
  return (
    ...
    <div className='flex justify-start items-center flex-col gap-9 bg-stone-100 h-max min-h-screen'>
        <NavbarStatic home={dict.home} contactUs={dict.contactUs} loginTitle={dict.loginTitle} bookingTitle={dict.bookingTitle} logoutTitle={dict.logoutTitle}  loggedIn={true}/>
        ...
        <div className='mb-20 w-full space-y-7'>
            {rides.map((ride, index) => (
                <Ride key={`ride ${ride.id}`} pickupText={pickup} destinationText={destination} pickup={ride.booking.pickup_location.name_location} destination={ride.booking.dropoff_location.name_location} date={ride.booking.datetime_pickup.split("T")[0]} time={ride.booking.datetime_pickup.split("T")[1].replace("Z", "")} status={ride.booking.status} price={ride.price}/>
            ))}
        </div>
    </div>
   }
